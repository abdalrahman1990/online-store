...
@section('content')
    ...
    <div class="container">
      ...
      <!-- Portfolio Section -->
      <h4><a href="{{ url('home') }}">Home page</a> <i class="text-info fa fa-angle-right"></i> {{ $category->category_name }}</h4>
      	<div class="row">
		            ...
		            <div class="card h-100">
		              <a href="{{ url('product/' . $product->id) }}"><img class="card-img-top" src="{{ asset('img/products/' . $product->product_image) }}" alt="product" height="350">
		                <div class="card-body">
		                  ...
		                  <p class="card-text" style="color: red"><b>Price: </b>{{ $price }} $</p>
		                  <p class="card-text"><b>Rateing: </b>{{ $product->product_rate}}</p>
		                </div>
		              </a>
		            </div>
		        </div>
